...
class Take {
  ...
  async takeOffer (cid) {
    try {
      ...
      const hex = await this.retryQueue.addToQueue(this.generatePartialTx, { offerData, utxoInfo })
      console.log('partial tx hex: ', hex)

      // Upload the Counter Offer to P2WDB
      const counterCid = await this.retryQueue.addToQueue(this.uploadCounterOffer, { offerData, hex, cid })
  ...
  async generatePartialTx (inObj = {}) {
    try {
      ...
      console.log('Error in generatePartialTx(): ', err)
      throw err
    }
  }
