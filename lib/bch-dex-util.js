...
class BchDexUtil {
  constructor (localConfig = {}) {
    ...
    if (!localConfig.bchWallet) {
      throw new Error('Instance of minimal-ecash-wallet must be passed as wallet property when instantiating xec-dex-util library.')
    }
    this.bchWallet = localConfig.bchWallet
    if (!localConfig.p2wdbRead) {
      throw new Error('Instance of p2wdb Read must be passed as p2wdbRead property when instantiating xec-dex-util library.')
    }
